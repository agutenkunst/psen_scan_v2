...
{
static constexpr std::size_t SIZE = 16;  // See protocol description
...
Message deserialize(const T& data)
{
  ...
  T tmp_data{ data };
  std::istringstream is(std::string(tmp_data.data(), SIZE));
  ...
  Type type = static_cast<Type>(opcode);
  if ((type != Type::start) && (type != Type::stop))
  {
    type = Type::unknown;
  }

  OperationResult result = static_cast<OperationResult>(res_code);
  if ((result != OperationResult::accepted) && (result != OperationResult::refused))
  {
    result = OperationResult::unknown;
  }

  Message msg{ type, result };

  return msg;
}

using RawType = FixedSizeRawData<SIZE>;
