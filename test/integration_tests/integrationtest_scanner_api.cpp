...
{
  scanner_reply::Type type = static_cast<scanner_reply::Type>(reply_type);
  scanner_reply::OperationResult result = scanner_reply::OperationResult::accepted;
  const scanner_reply::Message msg(type, result);
  control_server_.asyncSend<scanner_reply::SIZE>(control_msg_receiver_, scanner_reply::serialize(msg));
}

void ScannerMock::sendStartReply()
{
  std::cout << "ScannerMock: Send start reply..." << std::endl;
  sendReply(static_cast<uint32_t>(scanner_reply::Type::start));
}

void ScannerMock::sendStopReply()
{
  sendReply(static_cast<uint32_t>(scanner_reply::Type::stop));
}
...
TEST_F(ScannerAPITests, testStopFunctionality)
{
  ...
  EXPECT_CALL(scanner_mock, receiveControlMsg(_, serialize(stop_request::Message())))
